...
const isSquare = (arr) => {
  ...
  return arr.length? !arr.some((item) => {
...
const humanYearsCatYearsDogYears = (humanYears) => {
 ...
 return [humanYears, catYears.reduce((a, b) => a + b, 0), dogYears.reduce((a, b) => a + b, 0)]
};
...
function digitize(n) {
return String(n).split('').reverse().map(n => parseInt(n, 10))
}
2      
...
number = (busStops) => {
  ...
  return evenIn.reduce((a, b) => a + b, 0) - oddOut.reduce((a, b) => a + b, 0);
};
